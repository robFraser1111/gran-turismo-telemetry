...
internal static class Salsa20
{
    ...
    /// <summary>
    /// Decrypts <paramref name="cipher"/> in place by XOR'ing with the Salsa20 keystream.
    /// </summary>
    /// <param name="key">32-byte key.</param>
    /// <param name="nonce">8-byte nonce.</param>
    /// <param name="cipher">Ciphertext buffer (will be overwritten with plaintext).</param>
    public static void XorInPlace(ReadOnlySpan<byte> key, ReadOnlySpan<byte> nonce, Span<byte> cipher)
    {
        ...
        // Block counter (state[8..9]) starts at 0
        ...
        while (offset < cipher.Length)
        {
            ...
            // Increment 64-bit block counter (state[8] low, state[9] high)
            unchecked
            {
                state[8]++;
